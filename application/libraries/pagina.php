...
class pagina
{
	...
	function create_links()
	{
		...
		if($page_num > 1)
		{
			$output .= $this->first_tag_open.'<a '.$anchor_class.' href="'.$page_url.'1'.'" >首页</a>'.$this->first_tag_close;
		}
		//上一页
		if($page_num > 1)
		{
			$n = $page_num - 1;
			$output .= $this->prev_tag_open.'<a '.$anchor_class.' href="'.$page_url.$n.'" >上一页</a>'.$this->prev_tag_close;
		}
		//pages
		for($i=1;$i<=$pages;$i++)
		{
			...
			{
				$output .= $this->num_tag_open.'<a '.$anchor_class.' href="'.$page_url.$i.'" >'.$i.'</a>'.$this->num_tag_close;
			}
		}
		//下一页
		if($page_num < $pages)
		{
			$n = $page_num + 1;
			$output .= $this->next_tag_open.'<a '.$anchor_class.' href="'.$page_url.$n.'" >下一页</a>'.$this->next_tag_close;
		}
		//末页
		if($page_num < $pages)
		{
			$output .= $this->last_tag_open.'<a '.$anchor_class.' href="'.$page_url.$pages.'" >末页</a>'.$this->last_tag_close;
		}
